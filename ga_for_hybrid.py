...
class GA:
    def __init__(self, heatmap_values, possible_length, output_df, SRU, args, init_x, init_y):
    ...
    def initialization(self):
        ...
        for i in range(self.pop_size):
            population.append(self.generate_population(self.init_x, self.init_y))
        return population
    ...
    #             new_y = y + dy
    #
    #             # 이동했을 때 범위 안이면 변화량만 추가하고 좌표 갱신
    #             if 0 <= new_x < len(self.heatmap_values) and 0 <= new_y < len(self.heatmap_values[0]):
    #                 x, y = new_x, new_y
    #                 pop.append((dx, dy))  # 변화량만 저장
    #                 break
    #
    #     return pop
    ...
    def final_fitness(self, individual, total_coverage):
        # print("fitness")
        # print(f"individual = {individual}")
        # print(f"individual[0] = {individual[0]}")
        # path = self.convert_path(individual)
        ...
        return detect_point, heatmap_values_copy
    ...
    def evolve(self, logger, seed):
        ...
        fitness, heatmap = self.final_fitness(best_individual, total_coverage)
        print(f"best_individual: {best_individual}")
        return best_individual, heatmap
